...
def hero_def(name="promo_banner"):
...
class TestRegistry(unittest.TestCase):
    ...
    def test_register_and_merge(self):
        register_component_type("promo_banner", HERO_SCHEMA, "Hero")
    ...
    def test_request_components_override_global(self):
        register_component_type("promo_banner", HERO_SCHEMA, "global version")
        merged = merge_custom_types([{
            "name": "promo_banner",
            "data_schema": HERO_SCHEMA,
            "description": "request version",
        }])
        self.assertEqual(merged["promo_banner"].description, "request version")
...
class TestProviderSchemaExtension(unittest.TestCase):
    def test_custom_types_added_to_components_union(self):
        ...
        extended = zone_output_json_schema({"promo_banner": hero_def()})
        ...
        self.assertEqual(added["properties"]["type"]["const"], "promo_banner")
...
class TestValidateCustomComponents(unittest.TestCase):
    def setUp(self):
        self.custom = {"promo_banner": hero_def()}
    ...
    def test_builtins_still_work_alongside_custom(self):
        raw = [
            ...
            {"type": "promo_banner", "data": {"headline": "H"}},
        ]
        valid, errors = validate_components(raw, self.custom)
