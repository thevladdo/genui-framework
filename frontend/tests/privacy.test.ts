 ...
 * Migrated from the node:test harness to vitest: same contract,
...
const clickEvent = (target: any): any => ({
  clientX: 100,
  clientY: 100,
  target,
});
...
test("redactPII: emails, long numbers, IBAN, codice fiscale", () => {
  assert.equal(
    redactPII("write to mario.rossi@example.com now"),
    "write to [redacted] now",
  );
  assert.equal(redactPII("card 4111 1111 1111 1111 ok"), "card [redacted] ok");
  assert.equal(
    redactPII("IBAN IT60X0542811101000000123456"),
    "IBAN [redacted]",
  );
  assert.equal(
    redactPII("IBAN IT60 X054 2811 1010 0000 0123 456"),
    "IBAN [redacted]",
  );
...
test("redactPII: harmless text and short numbers survive", () => {
  assert.equal(
    redactPII("Only 4 items left at €1.299"),
    "Only 4 items left at €1.299",
  );
...
test("data-genui-redact: shape captured, content dropped", () => {
  ...
  t.handleClick(
    clickEvent(
      el({ textContent: "premium €12.000/year", href: "/quote/9" }, REDACT_SEL),
    ),
  );
  const sum = t.getCompactSummary();
...
test("form field content never captured, even with privacy off", () => {
  for (const privacy of ["strict", "balanced", "off"]) {
    ...
    t.handleClick(
      clickEvent(
        el({ tagName: "TEXTAREA", textContent: "my medical history" }),
      ),
    );
    const sum = t.getCompactSummary();
...
test("balanced (default): text captured but PII redacted", () => {
  ...
  t.handleClick(
    clickEvent(
      el({
        textContent:
          "Quote for mario.rossi@gmail.com — IBAN IT60X0542811101000000123456",
      }),
    ),
  );
...
test("strict: no free text leaves — no click text, no href, no title, no referrer", () => {
  ...
  t.handleClick(
    clickEvent(el({ textContent: "hello world", href: "https://x.com/u/1" })),
  );
...
test("navigation: path and title redacted in balanced", () => {
  const t = newTracker();
  t.trackNavigation(
    "/polizze/mario.rossi@gmail.com/rinnovo",
    "Rinnovo — mario.rossi@gmail.com",
  );
  const sum = t.getCompactSummary();
