/**
 * ChartComponent implementation (recharts lives here)
 *
 * Loaded on demand via React.lazy from ChartComponent.tsx so recharts
 * stays out of the entry bundle. Import THAT module, not this one.
 */
...
export const ChartComponent: React.FC<ChartComponentProps> = ({
  ...
  // Unknown chart type: same contract rule as unknown component types —
  // visible in dev, silently skipped in production (see ComponentRenderer).
  // Checked BEFORE ResponsiveContainer, which requires an element child.
  if (!chart) {
    console.warn(`GenUI: unknown chart type "${chartType}", skipping`);
    if (
